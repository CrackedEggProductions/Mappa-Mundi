# Bending Road

## 1. Tile name

**Bending Road** — production filename stem: `bending_road`. Art status: **HAS EXAMPLE**.

## 2. Authoritative role

Basic Act-I Expansion; connects adjacent Road sides. Starting bag: **4 copies**.

Source: [Complete Alpha Rules](../../../Carcassonne_Roguelite_Alpha_Complete_Rules.md), **RULE-CAT-010**; bag counts follow **RULE-BAG-002**. This brief defines art, not gameplay.

## 3. Edge occupancy

- North: **Road**
- East: **Road**
- South: **Field**
- West: **Field**

This is the production orientation, cross-checked against the existing Homestead content definition; the catalogue defines the topology, not a new fixed play orientation. Rotate the whole tile in 90-degree steps during play; do not mirror it.

## 4. Internal feature relationships

One continuous Road component joins north and east; south and west remain Field.

## 5. Visual communication goals

A broad handmade quarter-turn track joins the two centered sockets. Keep the surrounding parchment open. Follow the [master art recipe](../../specs/master_tile_art_spec.md): parchment first, fine sepia ink second, restrained color wash third. No scenic painting or 3D rendering.

## 6. Alignment requirements

- Use the [edge alignment specification](../../specs/tile_edge_alignment_spec.md): 1000 × 1000 design units, consistent edge clarity bands, no artwork frame that interrupts joins.
- Road sockets: exact edge midpoint, 100-unit width (10%); same width/treatment across all Road designs.
- Keep Field boundaries open and visually quiet; no unlisted feature may acquire an edge exit. Area-feature corner endpoints do not create an extra Field-side connection.

## 7. Color wash guidance

- Road: soft tan wash with fine sepia track boundaries.
- Field: faint desaturated light green; keep the parchment visible.

Wash stays low-saturation and translucent. Do not use decorative color to imply an unlisted gameplay feature.

## 8. Reference images to use

Inspect the entire [approved style set](../../references/approved_style/) first, then these relevant studies:

- [09 bending road](../../references/approved_style/ref_09_bending_road.png) — style anchor; [mechanical study](../../references/approved_mechanical/ref_09_bending_road.png) (conceptual topology only; recheck sockets).

North–west example rotates to north–east. Example orientation/width is subordinate to this brief and the alignment spec; “approved reference” does not mean final production art.

## 9. Forbidden mistakes

Do not fill the northeast corner as an area feature, branch the road, or extend it to south or west. Do not copy texture borders, off-center sockets or topology defects from a reference. Never let scenery imply additional feature exits.
